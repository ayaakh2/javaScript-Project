...
function add_user() {
    ...
    users.forEach((user) => {
        if (user.username == username.value) user_name_is_used = true;
    });
...
function Display_all_student() {
    ...
    if (!isFind) show.innerHTML = `Sorry, we couldn't find any students.`;
    else
...
function search_by_name(name) {
    ...
    users.forEach((user) => {
        if (
            String(user.fullName).includes(name) &&
...
function answer_by_exam(id) {
    ...
    results[id].answers.forEach((Answer, index) => {
        text += `<tr>
            <td>${Answer.questionId}</td>
            <td>${exams[id].questions[Answer.questionId].text}</td>
            <td>${Answer.answer}</td>
        </tr>`;
    });
